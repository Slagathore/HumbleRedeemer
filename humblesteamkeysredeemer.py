...
# Your Steam Web API info - the web app (app.py) keeps these in its Settings
# page instead; only fill these in if you use this legacy script standalone.
STEAM_API_KEY = ""
STEAM_ID_64 = ""

# Web API endpoint for owned games
...
def get_headless_driver():
    ...
    # This makes Humble think we are a real standard Windows Chrome browser
    ...
    for d,opt in possibleDrivers:
        try:
            ...
            if d == webdriver.Chrome:
                ...
                options.add_argument(f'user-agent={user_agent}') # Added this
            else:
                options.add_argument("-headless")
                options.set_preference("general.useragent.override", user_agent) # Added this
            ...
            continue
    cls()
    print("This script needs either Chrome or Firefox to be installed.")
    sys.exit()
...
def humble_login(driver):
    cls()
    # 1. Go to the login page first to set the domain context
    driver.get(HUMBLE_LOGIN_PAGE)
    
    # 2. Try to recover the saved session
    print("Attempting to restore Humble session...")
    if try_recover_cookies(".humblecookies", driver):
        # 3. Force navigation to the Library to see if it sticks
        driver.get(HUMBLE_KEYS_PAGE)
        time.sleep(2) # Give it a second to redirect if it fails
        
        # 4. If we are NOT on the login page, the cookie worked!
        if "login" not in driver.current_url and "onboarding" not in driver.current_url:
            print("Session restored successfully!")
            return True
        else:
            print("Saved session expired. Please log in again.")

    # Saved session didn't work (or didn't exist)
    authorized = False
    while not authorized:
        # NOTE: Go back to login page to ensure we are on the right screen
        driver.get(HUMBLE_LOGIN_PAGE)
        
        print("--- Humble Bundle Login ---")
        ...
        while "humble_guard_required" in login_json or "two_factor_required" in login_json:
            if "humble_guard_required" in login_json:
                ...
                if "user_terms_opt_in_data" in login_json and login_json["user_terms_opt_in_data"]["needs_to_opt_in"]:
                    print("There's been an update to the TOS, please sign in to Humble on your browser first.")
                    sys.exit()
...
def get_owned_apps(steam_session):
    """
    Simple owned-games lookup using the Steam Web API.

    Uses your STEAM_API_KEY and STEAM_ID_64 to call
    IPlayerService/GetOwnedGames with include_appinfo=1
    and returns a dict: {appid: game_name}.

    NOTE:
    - The steam_session parameter is kept for compatibility with
      the rest of the script, but this implementation does NOT
      actually use it (Web API calls don't need your cookies).
    """
    print("Fetching owned games via Steam Web API...")

    params = {
        "key": STEAM_API_KEY,
        "steamid": STEAM_ID_64,
        "include_appinfo": 1,            # get names!
        "include_played_free_games": 1,  # also count F2P you've played
        "format": "json",
    }

    try:
        resp = requests.get(STEAM_OWNED_GAMES_API, params=params, timeout=30)
        ...
        print(f"Found {len(owned_app_details)} owned games via Web API.")
        return owned_app_details

    except Exception as e:
        print("Error fetching owned games from Steam Web API:", e)
        # On failure, return empty; the rest of the script will just not
        # skip based on ownership (worst case, same behaviour as before
        # you had any ownership detection).
        return {}
